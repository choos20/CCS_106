...
$target = $challenge['target_matches']; // should be 20 for Daily Sprint
$grid_size = $challenge['grid_size'];   // should be 4
...
<head>
    ...
    <title>MindTwister · Daily Sprint</title>
...
<!-- Hidden audio elements (base64 encoded short sounds) -->
...
<script>
    ...
    let seconds = 0;
    let timerStarted = false; // Start timer on first flip
                ...
                if (cards.every(c => c.matched)) {
                    setTimeout(() => {
                        if (!completed && currentProgress < target) {
                            createDeck();  // fresh shuffled deck
                            renderGrid();
                        }
                    }, 500);
                }
